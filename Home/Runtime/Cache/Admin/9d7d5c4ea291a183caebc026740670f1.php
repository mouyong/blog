...
<div id="content">
    <a href="/blog/Snail/Index/save">添加博文</a>
	<table>
		<tr>
			<th>标题</th>
			<th>类型</th>
		...
		<?php if(is_array($article)): $i = 0; $__LIST__ = $article;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?><tr>
			...
			<td class="first"><?php echo (stristr($article["article"], '</p>', true)); ?></td>
			<td>
			<?php switch($article["display"]): case "1": ?>显示<?php break;?>
				<?php case "0": ?><span class="ad">隐藏</span><?php break; endswitch;?>
			</td>
			<td>
				<a href="/blog/Snail/Index/modify/id/<?php echo ($article["id"]); ?>">修改</a>
				<a href="/blog/Snail/Index/del/id/<?php echo ($article["id"]); ?>" onclick="return confirm('真的要删除吗?此操作不可恢复!')">删除</a>
			</td>
